...
void createCircle(CircleGeometry &givenCircle, int segments, std::vector<CircleInstance> &storage)
{
    ...
    {
        storage.push_back({glm::vec3(givenCircle.center.x, givenCircle.center.y, 0.0f)});
        ...
        storage.push_back({glm::vec3(xi, yi, 0.0f)});
        ...
        storage.push_back({glm::vec3(xip1, yip1, 0.0f)});
    }
}
